...
function updateLocalUsers(
  ...
  const otherUsers = users.filter((user) => {
    if (user.name != current.name && user.email != current.email) return true;
...
function storeToHistory() {
  if (localStorage.getItem("loggedIn")) {
    ...
    const today = `${
      date.getMonth() + 1
    }/${date.getDate()}/${date.getFullYear()}/${date.getMilliseconds()}`;
    history[today] = order;
...
orderBtn.addEventListener("click", function () {
  if (checkLoggin()) {
    ...
    isOrder();
    alert(
      "Thank you for ordering! Your order will be ready for pickup in 40 min!"
    );
  } else {
    clearCurrentOrder();
    isOrder();
